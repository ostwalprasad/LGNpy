...
def test_learning_and_inference():
    ...
    np.testing.assert_almost_equal(
        list(np.diag(np.array(lg.get_covariance()))),
        [
            3.29907957452064,
            8.18536047354675,
            84.70325221116876,
    ...
    np.testing.assert_almost_equal(
        list(np.diag(np.array(lg.get_precision_matrix()))),
        [
            1.422843910211299,
            2.141985350397759,
            2.29749879236002,
    ...
    np.testing.assert_almost_equal(
        pc, [[0.21821993825026098, 2.2260893644210045, 2.9918154822098213]], decimal=3
    )
    np.testing.assert_almost_equal(
        pe, [[-1.1154965220785016, 3.0272578114505975, 0.2696565138830367]], decimal=3
    )
